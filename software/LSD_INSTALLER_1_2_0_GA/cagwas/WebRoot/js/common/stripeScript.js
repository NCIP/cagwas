 ...
 stripe : function(id) {
    ...
    var mo = (arguments[3] && arguments[3] == "false")? false : true;
...
//        if (! this.hasClass(trs[i]) && ! trs[i].style.backgroundColor) {
        ...
        //add the mouseovers and outs... RCL
			...
			if(mo && i>0)	{
				...
				trs[i].onmouseover = function()	{Stripe.highlight(this) };
				//trs[i].setAttribute("onmouseout", Stripe.unhighlight(this));
				trs[i].onmouseout = function() { Stripe.unhighlight(this)};
			}
  ...
  highlight : function(item)	{
  	item.name = item.style.backgroundColor;
	item.style.backgroundColor = "#CEFBFF";
  },
  
  unhighlight : function(item)	{
  	if(item.name)
	item.style.backgroundColor  = item.name;
  }
